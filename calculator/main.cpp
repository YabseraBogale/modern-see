...
int main() {
    ...
    CROW_ROUTE(app,"/chat").methods(crow::HTTPMethod::GET,crow::HTTPMethod::POST)([](const crow::request& request,const crow::response& response){
        if(request.method==crow::HTTPMethod::POST){

        } else if(request.method==crow::HTTPMethod::GET){

        } else{
            return std::to_string(501);
        }
    });
